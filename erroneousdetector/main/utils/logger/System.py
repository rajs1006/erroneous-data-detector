...
from erroneousdetector.main.utils.decorator.Singleton import Singleton
...
def logger(fileName: str) -> (logging.log, bokeh_logger):
    """
    Create and return Singleton instance of python and Bokeh logger.\n
    Args:
        name {str} -- name of the file from where the method hs been called.
    Returns:
        System logger and bokeh logger
        system logger : used to log the system logs, like time of methods,
                        some print statement inside a method etc.
        bokeh logger : used to log bokeh events, like session created,
                        session destroyed etc.
    """
    ...
    class Logger:
        def __init__(self):
            ...
            self.handler.setFormatter(
                JsonFormatter(STRING_FORMAT, record_custom_attrs=CUSTOM_FORMAT)
            )
